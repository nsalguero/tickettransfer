...
class PluginTickettransferTickettab extends CommonDBTM {
   ...
   /**
    * Détermine si le ticket peut être transféré par cet utilisateur.
    * Cela nécessite les droits pour : - modifier le
   ...
   static function canTansfer(Ticket $ticket) {
      ...
      return $ticket->can($ticket->getID(), UPDATE) && $fup->can(-1, CREATE, $fuptestinput) && ($config['allow_transfer'] && !empty($config['allowed_entities']) || $config['allow_group']);
   }
