...
class PluginTickettransferEntitiesdropdown {
    ...
    * @param
    *           array options options facultatives:
   ...
   static function entitiesDropdown($entities_list, $options = array()) {
      ...
      if ($result = $DB->query($query)) {
         echo '<select id="dropdown_' . $params['name'] . $params['rand'] . '" name="' . $params['name'] . '" size="1">';
   ...
   private static function getOptionString($entityfields, $selected = false, $disabled = false) {
      ...
      $ret = '<option ' . ($disabled ? 'disabled ' : '') . ($selected ? 'selected ' : '') . 'value="' . $entityfields['id'] . '" ' . $class . ' title="' . Html::cleanInputText($title) . '">';
